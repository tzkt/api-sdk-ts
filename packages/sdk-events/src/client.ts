...
export type OperationObservable = Observable<
  SubscriptionMessage<TezosOperation>
>;
export type BigMapObservable = Observable<SubscriptionMessage<BigMapUpdate>>;
export type TokenTransferObservable = Observable<
  SubscriptionMessage<TokenTransfer>
>;
...
export class EventsService {
  ...
  private statusObservers: Set<
    ZenObservable.Observer<signalR.HubConnectionState>
  >;
  ...
  constructor({ url, reconnect = true }: EventsConfig) {
    ...
    this.connection.on(CHANNEL.OPERATIONS, (msg) =>
      this.onMessage(CHANNEL.OPERATIONS, msg)
    );
    this.connection.on(CHANNEL.BIGMAPS, (msg) =>
      this.onMessage(CHANNEL.BIGMAPS, msg)
    );
    this.connection.on(CHANNEL.TRANSFERS, (msg) =>
      this.onMessage(CHANNEL.TRANSFERS, msg)
    );

    this.connection.onclose(() =>
      this.onStatusChanged(signalR.HubConnectionState.Disconnected)
    );
    this.connection.onreconnecting(() =>
      this.onStatusChanged(signalR.HubConnectionState.Reconnecting)
    );
    ...
    this.statusChanges = new Observable<signalR.HubConnectionState>(
      (observer) => {
  ...
  public async start(): Promise<void> {
    switch (this.connection.state) {
      ...
      default:
        throw new Error(
          `Intermediate connection hub state: ${this.connection.state}`
        );
    }
  }
   ...
   */
  public operations(
    params: OperationSubscriptionParameters
  ): OperationObservable {
    return new Observable<SubscriptionMessage<TezosOperation>>((observer) => {
      return this.createSubscription<TezosOperation>(
        CHANNEL.OPERATIONS,
        observer,
        params
      );
  ...
  public bigmaps(params: BigMapSubscriptionParameters): BigMapObservable {
    return new Observable<SubscriptionMessage<BigMapUpdate>>((observer) => {
      return this.createSubscription<BigMapUpdate>(
        CHANNEL.BIGMAPS,
        observer,
        params
      );
...
class Subscription<Type> {
      ...
      switch (channel) {
        case CHANNEL.OPERATIONS:
          return this.matchOperation(
            ...
            this.params as OperationSubscriptionParameters
          );
        case CHANNEL.BIGMAPS:
          return this.matchBigMapUpdate(
            item as BigMapUpdate,
            this.params as BigMapSubscriptionParameters
          );
        case CHANNEL.TRANSFERS:
          return this.matchTokenTransfer(
            item as TokenTransfer,
            this.params as TokenTransferSubscriptionParameters
          );
      }
    }
    return true;
  ...
  private matchOperation(
    ...
    if (
      params.types &&
      !params.types.includes(operation.type as OperationKind)
    ) {
      return false;
    }
    if (params.address) {
      switch (operation.type) {
        case 'endorsement':
          return (
            (operation as EndorsementOperation).delegate?.address ===
            params.address
          );
        case 'preendorsement':
          return (
            (operation as PreendorsementOperation).delegate?.address ===
            params.address
          );
        case 'ballot':
          return (
            (operation as BallotOperation).delegate?.address === params.address
          );
        case 'proposal':
          return (
            (operation as ProposalOperation).delegate?.address ===
            params.address
          );
        case 'activation':
          return (
            (operation as ActivationOperation).account?.address ===
            params.address
          );
        case 'double_baking': {
          const op = operation as DoubleBakingOperation;
          return (
            op.offender?.address === params.address ||
            op.accuser?.address === params.address
          );
        }
        ...
        case 'nonce_revelation': {
          ...
          return (
            op.baker?.address === params.address ||
            op.sender?.address === params.address
          );
        }
        case 'delegation':
          return (
            (operation as DelegationOperation).sender?.address ===
            params.address
          );
        case 'origination': {
          ...
          return (
            op.sender?.address === params.address ||
            op.originatedContract?.address === params.address
          );
        }
        case 'transaction': {
          ...
          return (
            op.sender?.address === params.address ||
            op.target?.address === params.address
          );
        }
        case 'reveal':
          return (
            (operation as RevealOperation).sender?.address === params.address
          );
        case 'register_constant':
          return (
            (operation as RegisterConstantOperation).sender?.address ===
            params.address
          );
        case 'set_deposits_limit':
          return (
            (operation as SetDepositsLimitOperation).sender?.address ===
            params.address
          );
        case 'migration':
          return (
            (operation as MigrationOperation).account?.address ===
            params.address
          );
        case 'revelation_penalty':
          return (
            (operation as RevelationPenaltyOperation).baker?.address ===
            params.address
          );
        case 'baking': {
          const op = operation as BakingOperation;
          return (
            op.proposer?.address === params.address ||
            op.producer?.address === params.address
          );
        }
        case 'endorsing_reward':
          return (
            (operation as EndorsingRewardOperation).baker?.address ===
            params.address
          );
      }
    }
  ...
  private matchBigMapUpdate(
    update: BigMapUpdate,
    params: BigMapSubscriptionParameters
  ): boolean {
    if (params.contract) {
      if (params.contract !== update.contract) {
        return false;
      }
    }
    if (params.ptr) {
      if (params.ptr !== update.bigmap) {
        return false;
      }
    }
    if (params.path) {
      if (!update.path?.startsWith(params.path)) {
        return false;
      }
    }
    // TODO: filter by tags
  ...
  private matchTokenTransfer(
    transfer: TokenTransfer,
    params: TokenTransferSubscriptionParameters
  ): boolean {
    if (params.account) {
      if (params.account !== transfer.from && params.account !== transfer.to) {
        return false;
      }
    }
    if (params.contract) {
      if (params.contract !== transfer.token?.contract) {
        return false;
      }
    }
    if (params.tokenId) {
      if (params.tokenId !== transfer.token?.tokenId) {
        return false;
      }
    }
    return true;
